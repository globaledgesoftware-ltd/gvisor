...
func TestSynWithSameLocalRemoteIP(t *testing.T) {
	...
	// Extracting dstaddr and configuring this as srcaddr
	dstaddr := *conn.Outgoing[1].(*tb.IPv4).DstAddr
	layersToSend := deepcopy.Copy(conn.Outgoing).(tb.Layers)
	if err := layersToSend[ipv4LayerIndex].(*tb.IPv4).Mergelayer(tb.IPv4{SrcAddr: &dstaddr}); err != nil {
		t.Fatalf("can't merge", err)
	}

        // Sending TCP SYN Message to DUT
	if err := layersToSend[tcpLayerIndex].(*tb.TCP).Mergelayer(tb.TCP{Flags: tb.Uint8(header.TCPFlagSyn)}); err != nil {
		t.Fatalf("can't merge", err)
	}
